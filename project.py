...
bot = telebot.TeleBot(
    'what you need here?', parse_mode='html')
...
def date(message) -> None:
    try:
        ...
        bot.send_message(
            message.chat.id, 'Я конечно люблю программировать, но не настолько, чтобы решать данную проблему👨‍💻', reply_markup=globalkey())
...
def infos(message) -> None:
    try:
        ...
        bot.send_message(
            message.chat.id, 'Тебя куда-то не туда занесло, вернись лучше в главное меню ☺️', reply_markup=globalkey())
...
def starosta(message) -> None:
    try:
        ...
        bot.send_message(
            message.chat.id, 'Братишка, ты что-то не то делаешь👮🏼‍♂️', reply_markup=globalkey())
